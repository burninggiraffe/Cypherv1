...
stringEnter:;
	...
	std::cout << "Enter the 16bit hex plain text \n";
	cin >> p;
	if (p.length() != 16)
	{
		std::cout << "enter all the bits\n";
		goto stringEnter;
	}
	for (int i = 0; i<16; i++)
	{
		...
		{
			std::cout << "Not a valid hexadecimal string\n";
			goto stringEnter;
		}
	}
	int key1[64];
keyEnter:;
	std::cout << "Enter the 16bit hex key \n";
	cin >> ke;
	if (ke.length() != 16)
	{
		std::cout << "enter all the key bits\n";
		goto keyEnter;
	}
	for (int i = 0; i<16; i++)
	{
		...
		{
			std::cout << "Not a valid hexadecimal key string\n";
			goto keyEnter;
		}
	}
	//Input is completed
	...
	r = p.substr(32, 32);
	//key generation
	int i, t = 1, j, row, col, temp, rnd = 16;
	j = 0;
	for (i = 0; i<56; i++)
	...
	while (rnd--)
	{
		...
		ep += r[0];
		//Xor key with output of p box
		for (i = 0; i<48; i++)
			xorout += char(((int(ep[i]) - 48) ^ key[16 - rnd - 1][i]) + 48);
		//sbox compression 48 -> 32
		for (i = 0; i<48; i += 6)
		{
		...
		l = rtem;
		std::cout << "Output after Round " << 16 - rnd << endl;
		string cip = "";
		for (i = 0; i<32; i += 4)
		{
			int te;
	...
	*/
	...
	while (rnd--)
	{
		...
		for (i = 0; i<48; i++)
			xorout += char(((int(ep[i]) - 48) ^ key[rnd][i]) + 48);
		...
		r = ltem;
		std::cout << "Decrypted Output after Round " << 16 - rnd << endl;
		string cip = "";
		for (i = 0; i<32; i += 4)
		{
			int te;
